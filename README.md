...
```text
东方财富 K 线（默认，外部 API + 本页图表）
新浪财经（新窗口）
腾讯证券（新窗口）
TradingView（备选）
```

输入 `000001.SZ`、`sz000001` 或 `000001` 都会自动映射到外部源需要的代码格式。默认东财 K 线直接请求东方财富公网接口并在本页绘制 `1m/5m/15m/30m/60m/1d`，因此不会出现 TradingView 角标。新浪/腾讯网页经常限制 iframe 嵌入，前端改为新窗口打开，避免页面空白。TradingView 的 A 股分钟级别可能受数据授权限制而提示 unsupported，且其官方组件署名不能被隐藏或删除。
